...
export interface DocumentListStyles {
  bootomContainerStyles: IStackStyles;
  documentListStyles: IProcessedStyleSet<{
    fileIconHeaderIcon: string;
    fileIconCell: string;
    fileIconImg: string;
    controlWrapper: string;
    selectionDetails: string;
    filesContainerGrid: string;
    noDataFoundStyles: IStyle;
    separator: string;
    documentList: string;
  }>;
  scollableContainerStyles: Partial<IScrollablePaneStyles>;
}

export const useDocumentListStyles = (): DocumentListStyles => {
  ...
  const containerHeight = React.useMemo(() => {
    return containerWidth && files.length
      ? (Math.ceil(pageSize ?? DEFAULT_PAGE_SIZE) /
          Math.ceil(containerWidth / 180)) *
          180
      : 0;
  ...
  const scollableContainerStyles: Partial<IScrollablePaneStyles> =
    React.useMemo(() => {
      return {
        root: {
          position: 'relative',
